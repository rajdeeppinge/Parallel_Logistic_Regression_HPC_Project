...
Here a possible improvement of regularization has been added to avoid overfitting.

This code is a parallel implementation of the corresponding serial code
*/
...
//	float alpha = 0.002; //this is called learning rate to update theta vector. This value is found out experimentally
	...
	float reg_term = 0.1; // this is called regularization term. It is used to avoid overfitting and false predictions
...
//	double partial_grad[n];
	...
	# pragma omp parallel for private (j, x_viewrow) 
	for (i = 0; i < m; i++)
	{
	...
	for (j = 0; j < n ;j++)
	{
		...
		gsl_vector_float_set (&theta.vector, j, gsl_vector_float_get(&theta.vector, j)-(alpha*( (partial_grad[j] + reg_term * gsl_vector_float_get(&theta.vector, j)) /m)) );
	}
}
...
// iteration=100;		// iterating 1e6 times to obtain theta vector as accurate as possible

...
{  
	...
	double final_grad = 0.0, partial_grad = 0.0;
	...
	for (j = 0; j < n ;j++)
	{
		...
		gsl_matrix_float_get_col(X_j, X, j);	// get j'th column from matrix X and store it in global vector variable X_j

		#pragma omp parallel
        {
			partial_grad=0.0;    

			#pragma omp for private(i, x_row) reduction(+:final_grad)
			// loop which calculates gradient of cost function for given theta vector
			for (i = 0; i < m; i++)
			{
				x_row= gsl_matrix_float_row (X,i); // can drop this later too , pass directly as &
				
				//gsl_matrix_float_get_row(x_row, X, i);		// get i'th row of matrix X
				
				partial_grad += ( ( sigmoid(&x_row.vector, theta) - gsl_vector_get(y, i) ) * gsl_vector_float_get(X_j, i) );   
...
void train(gsl_vector_float* theta, gsl_vector* y,gsl_matrix_float* X)
{
	...
	#pragma omp parallel
	{
		...
		int n = X->size2, m = X->size1, j;	// for our example n = 3, m = 100
		...
		for (ite = 1; ite <= iteration; ite++)
		{
			for (j = 0; j < n ;j++)
			{
				...
				// loop which calculates gradient of cost function for given theta vector
				#pragma omp parallel for reduction(+:final_grad)
				for (i = 0; i < m; i++)
				{
					x_row = gsl_matrix_float_row (X,i); // can drop this later too , pass directly as &

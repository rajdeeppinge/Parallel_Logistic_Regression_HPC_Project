...
This code parallelizes the outer loop of training.
*/
...
/* This function finds the best possible parameters of vector theta such that the cost function which is the cost incurred by the programme due to a wrong prediction is minimized. Currently this function is included in the train itself to reduce function call overhead.
...
//	float alpha = 0.1; //this is called learning rate to update theta vector. This value is found out experimentally
	...
	for (j = 0; j < n ;j++)		// run loop over all features i.e. n
	{
		...
		gsl_vector_float_set (theta, j, gsl_vector_float_get(theta, j)-(alpha*(partial_grad/m)) );
	}
...
/*// for testing purpose	 
  // checking if predict is correct, ignore time at present
